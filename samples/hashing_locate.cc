...
int main(int argc, char** argv){
	if(argc!=11){cout<< argv[0] << " data_file UpperBits BaseCodeFile query_file QueryCodeFile tableNum codelen radius lenshift initsz" <<endl; exit(-1);}
  ...
  int tableNum = atoi(argv[6]);
  int codelen = atoi(argv[7]);
  int radius = atoi(argv[8]);
  int lenshift = atoi(argv[9]);
  ...
  index.locatPoints(tableNum/*use hashing table*/,query);
