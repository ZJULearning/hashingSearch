...
public:
	...
	void locateNeighbors(size_t nT, const Matrix<DataType>& query){
		if (codelength <= 32 ){
			locateNeighbors32(nT,query);
		}else if(codelength <= 64 ){
			locateNeighbors64(nT,query);
		}else{
			std::cout<<"code length not supported yet!"<<std::endl;
		}
	}

	void locateNeighbors32(size_t nT, const Matrix<DataType>& query){
	...
	void locateNeighbors64(size_t nT, const Matrix<DataType>& query){
		int lowbits = codelength - upbits;
